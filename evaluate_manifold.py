...
tf.app.flags.DEFINE_string("which", True, "Which evalutaion set to use")
...
def main(_):
    ...
    shdata = np.load("data/sh.npz")
    FLAGS.window_length = (shdata[FLAGS.which].shape[1]//16)*16
    ...
    def project_to_2d(x):
        return (x, tf.zeros(shape=[tf.shape(x)[0], tf.shape(x)[1] ,73]))
    ...
    hourglass_detections = tf.constant(shdata[FLAGS.which][:,0:FLAGS.window_length].astype(np.float32))
    dataset = tf.data.Dataset.from_tensors(hourglass_detections)
    ...
    result_output_x3d= sess.run(autoencoded, {
        independent: FLAGS.independent,
        procrustes: FLAGS.procrustes,
        hourglass.training: False,
        manifold.training: False
    ...
    np.savez(os.path.join(train_dir, FLAGS.which+".npz"), x3dout = result_output_x3d)
